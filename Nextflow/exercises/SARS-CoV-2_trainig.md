...
```
ls results/variants/varscan2/*.vcf.gz | cut -d "/" -f4 | cut -d "." -f1 | sort -u > samples_id.txt
```
...
```
cat samples_id.txt | while read in; do cat results/variants/bam/samtools_stats/${in}.sorted.bam.flagstat | grep '+ 0 in total' | cut -d " " -f1; done
```

For the next steps we will store this information in a variable
```
total_reads=$(cat samples_id.txt | while read in; do cat results/variants/bam/samtools_stats/${in}.sorted.bam.flagstat | grep '+ 0 in total' | cut -d " " -f1; done)
```
...
```
cat samples_id.txt | while read in; do cat results/variants/bam/samtools_stats/${in}.sorted.bam.flagstat | grep '+ 0 mapped' | cut -d " " -f1; done
```
For the percentage:
```
cat samples_id.txt | while read in; do cat results/variants/bam/samtools_stats/${in}.sorted.bam.flagstat | grep '+ 0 mapped' | cut -d "(" -f2 | cut -d "%" -f1; done
```

An then store them in variables:
```
mapped_reads=$(cat samples_id.txt | while read in; do cat results/variants/bam/samtools_stats/${in}.sorted.bam.flagstat | grep '+ 0 mapped' | cut -d " " -f1; done)
percentage_mapped_reads=$(cat samples_id.txt | while read in; do cat results/variants/bam/samtools_stats/${in}.sorted.bam.flagstat | grep '+ 0 mapped' | cut -d "(" -f2 | cut -d "%" -f1; done)
```
